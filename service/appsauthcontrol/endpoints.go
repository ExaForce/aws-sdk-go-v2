...
import (
	...
	internalendpoints "github.com/aws/aws-sdk-go-v2/service/appsauthcontrol/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_APPS_AUTH_CONTROL")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Apps Auth Control", cfg.ConfigSources)
	if found && err == nil {
...
func (r *resolver) ResolveEndpoint(
	...
	if exprVal := params.Endpoint; exprVal != nil {
		...
		if _UseDualStack == true {
			return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Custom endpoint or DualStack is not supported")
		}
		if _UseFIPS == true {
			return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Custom endpoint or FIPS is not supported")
		}
		return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Custom endpoint is not supported")
	}
	if _UseDualStack == true {
		if _UseFIPS == true {
			return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: DualStack or FIPS is not supported")
		}
		return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: DualStack is not supported")
	}
	if _UseFIPS == true {
		return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: FIPS is not supported")
	}
	if exprVal := params.Region; exprVal != nil {
		...
		if _Region == "cn-north-1" {
			uriString := func() string {
				...
				out.WriteString("https://auth-control.")
				out.WriteString(_Region)
				out.WriteString(".prod.apps-auth.aws.a2z.com.cn/")
				return out.String()
			}()
		...
		if _Region == "us-isob-east-1" {
			return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Region is not supported")
		}
		if _Region == "us-iso-east-1" {
			return endpoint, fmt.Errorf("endpoint rule error, %s", "Invalid Configuration: Region is not supported")
		}
		uriString := func() string {
			var out strings.Builder
			out.WriteString("https://auth-control.")
			out.WriteString(_Region)
			out.WriteString(".prod.apps-auth.aws.a2z.com/")
			return out.String()
		}()
