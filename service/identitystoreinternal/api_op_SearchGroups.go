...
// Search all groups in the identity store. Returns a paginated list of complete
// Group objects.
...
type SearchGroupsInput struct {
	...
	// A list of Filter objects, which is used in the SearchGroups request.
	Filters []types.Filter

	// The maximum number of results to be returned per request. This parameter is
	// used in the SearchGroups request to specify how many results to return in one
	// page. The length limit is 50 characters.
	MaxResults *int32

	// The pagination token used for the SearchGroups API operation. This value is
...
func (c *Client) addOperationSearchGroupsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	err = stack.Serialize.Add(&awsAwsjson11_serializeOpSearchGroups{}, middleware.After)
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(&awsAwsjson11_deserializeOpSearchGroups{}, middleware.After)
	if err != nil {
		return err
	}
